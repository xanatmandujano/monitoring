...
const streamingUrl = url.server.streamingUrl;
...
export async function webRTC(elementName = null, deviceId = null, rtcp = null) {
  ...
  peerConn.onnegotiationneeded = async () => {
    ...
    const res = await fetch(`${streamingUrl}/stream/receiver/${deviceId}`, {
      method: "POST",
      headers: {
    ...
    if (data.length === 0) {
      log("cannot connect:", `${streamingUrl}`);
    } else {
  ...
  const res = await fetch(`${streamingUrl}/stream/codec/${deviceId}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
    },
  ...
  dataChannel.onerror = (e) =>
    log("channel error:", channel.label, "payload", e);
